...
def __sum_over(var_name, weight):
	return lambda variables: weight * reduce(lambda a, b :a + b, variables[param_name])
...
#  Minimize angular momentum variation
#  \return  cone matrices for each phase
def min_dcc(param, weight):
	return __sum_over('dcc', weight)
...
def end_reached(param, weight):
	...
	x_size = len(x_end)
	return lambda variables : weight * norm(variables["x"][-x_size:] - x_end)
...
def init_objective(objective, params):
	...
	cost_functions = [__constraint_factory[name](params, weight) for name, weight in objective]
	def objective_fun (u):
